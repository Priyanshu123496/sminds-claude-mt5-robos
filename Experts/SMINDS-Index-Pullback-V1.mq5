...
input long   InpMagicNumber       = 94736345;        // Magic (Index-Pullback distinct)
input string InpOrderComment      = "IPB-V1";        // Order comment

input group "═══ Symbol / Timeframe Guard ══════════════════════"
input bool   InpRequireXAUUSD     = false;           // Indices: disable XAU guard
input bool   InpRequireM15        = true;            // Keep M15
...
input double InpMinADX            = 18.0;            // ADX must be ≥ this (looser for indices)
...
input double InpMinSlopeATR       = 0.2;             // Min |EMA50 slope| / ATR (looser for indices)

input group "═══ Pullback Detection ═══════════════════════════"
input int    InpMinPullbackBars   = 1;               // ≥ this many bars retracing (looser)
input double InpPullbackToEMAATR  = 1.0;             // Within 1.0×ATR of EMA20 (looser for indices)
...
//| Constants & state                                                 |
//+------------------------------------------------------------------+
#define EA_TAG     "IPB-V1"
#define GV_PREFIX  "IPBV1_"
