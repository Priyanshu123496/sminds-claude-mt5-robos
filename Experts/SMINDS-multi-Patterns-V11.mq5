...
input long   InpMagicNumber       = 95001011;        // Magic number
input string InpOrderComment      = "SMINDS-MP-V11";  // Order comment
...
input double InpRRRatio           = 1.5;             // V11: 1.5:1 — prioritize win rate per user spec

input group "═══ Trade Management (V4) ════════════════════════"
input bool   InpUsePartialClose   = false;           // V10: pure 3:1 SL/TP
...
input bool   InpUseEngulfing      = false;           // V9: chart patterns OFF (use EMA pullback)
input bool   InpUsePinBar         = false;
input bool   InpUseStar           = false;
input bool   InpUseInsideBar      = false;
input bool   InpUseEMAPullback    = true;            // V9: NEW — EMA pullback continuation
input int    InpPullbackBars      = 4;               // # bars to confirm pullback occurred
...
void OnTick()
{
   ...
   // ── Pattern scan ──
   PatternHit hit;
   hit.dir = 0; hit.extreme = 0.0; hit.name = ""; hit.trigger = 0.0;
   bool found = false;
   if(InpUseEMAPullback) found = DetectEMAPullback(atr, htf_dir, hit);
   if(!found) found = ScanPatterns(atr, hit);
   if(!found) return;
