//+------------------------------------------------------------------+
//|                                  TTR-XI-V1a-prod-ready-v12.mq5  |
//|                                       Sminds Global Inc. (c)2026 |
//|                                                                  |
//|  Production-ready release based on TTR-XI-V1a-Improved-V12.      |
//|                                                                  |
...
#property description "TTR-XI V1a — production-ready V12"
#property description "Triple-EMA (9/20/200) + dynamic lot DD guard"
#property description "XAUUSD M15 — verified Q1+April 2026 positive"
#property strict
...
input long   InpMagicNumber       = 94736312;        // Magic number (per-account unique)
input string InpOrderComment      = "TTR-XI-V12";    // Order comment
...
input double InpMaxLotSize        = 1.00;            // Hard cap on lot size (safety)
...
//| Constants & state                                                 |
//+------------------------------------------------------------------+
#define EA_TAG          "TTR-XI-V12"
#define GV_PREFIX       "TTRXIV12_"
...
//+------------------------------------------------------------------+
//| Dynamic lot sizing (DD-aware) with deposit/withdrawal protection  |
//+------------------------------------------------------------------+
double GetEffectiveLot()
{
   if(!InpUseDynLots)
      return MathMin(InpLotSize, InpMaxLotSize);

   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   double equity  = AccountInfoDouble(ACCOUNT_EQUITY);

   // Detect external deposit/withdrawal and recalibrate peak so the
   // DD computation does not flip false-positive after a top-up
   // or false-negative after a withdrawal.
   if(g_last_balance > 0.0)
   {
      double bal_change = balance - g_last_balance;
      // Use 0.1% threshold to ignore P/L-driven balance moves from
      // closed trades — they are reflected by RecordTradeResult.
      // External funding changes are usually round, large numbers.
   ...
   double lot = g_lots_halved ? (InpLotSize * 0.5) : InpLotSize;
   return MathMin(lot, InpMaxLotSize);
}
...
bool OpenBuy(const double atr)
{
   ...
   double vol = NormalizeVol(GetEffectiveLot());
   if(vol <= 0.0)
   {
   ...
   if(ok)
      LogInfo(StringFormat("BUY %.2f @ %.5f SL=%.5f (DD-half=%s)",
                           vol, ask, sl, g_lots_halved ? "Y" : "N"));
   return ok;
}
...
bool OpenSell(const double atr)
{
   ...
   if(ok)
      LogInfo(StringFormat("SELL %.2f @ %.5f SL=%.5f (DD-half=%s)",
                           vol, bid, sl, g_lots_halved ? "Y" : "N"));
   return ok;
}
...
int OnInit()
{
   ...
   if(InpLotSize <= 0.0 || InpMaxLotSize <= 0.0 || InpMaxLotSize < InpLotSize)
   {
      LogError("Invalid lot configuration: InpLotSize and InpMaxLotSize must be > 0 and InpMaxLotSize >= InpLotSize");
      return INIT_PARAMETERS_INCORRECT;
   }
   ...
   LogInfo(StringFormat("Lot            : base=%.2f max=%.2f dyn=%s halve@%.0f%% restore@%.0f%%",
                        InpLotSize, InpMaxLotSize, InpUseDynLots ? "ON" : "OFF",
