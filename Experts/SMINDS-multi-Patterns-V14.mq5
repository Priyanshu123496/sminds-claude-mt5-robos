...
input long   InpMagicNumber       = 95001014;        // Magic number
input string InpOrderComment      = "SMINDS-MP-V14";  // Order comment
...
input double InpRiskPercent       = 2.0;             // V14: pushed to 2% (within user's 1.5-2% spec) for higher profit
input double InpMaxLotSize        = 3.00;            // V14: allow more compounding
...
input double InpRRRatio           = 2.0;             // V12: 2:1 — confluence supports more achievable target

input group "═══ Trade Management (V4) ════════════════════════"
input bool   InpUsePartialClose   = true;            // Close half at 1R, runner to TP
input double InpPartialAtR        = 1.0;             // Partial close trigger (× initial SL distance)
input double InpPartialFraction   = 0.5;             // Fraction of position to close at partial trigger
input bool   InpUseBreakEven      = true;            // Move SL to entry+/- offset after partial trigger
input double InpBreakEvenOffsetR  = 0.0;             // V6: pure BE (entry), don't penalize runners

input group "═══ Stop Loss Placement ══════════════════════════"
input int    InpATRPeriod         = 14;              // ATR period
input double InpATRBufferMult     = 0.5;             // SL extra buffer = ATR × this beyond pattern extreme
input double InpMaxSLDistATR      = 1.5;             // V3: Cap SL distance at this × ATR (prevents huge pin-wick stops)
input bool   InpRequireBreakout   = false;           // V9: disabled for EMA pullback (close above EMA is enough)

input group "═══ Pattern Selection ═════════════════════════════"
input bool   InpUseEngulfing      = true;            // V12: ON — required confluence
input bool   InpUsePinBar         = true;            // V12: ON — required confluence
input bool   InpUseStar           = true;            // V12: ON — required confluence
input bool   InpUseInsideBar      = false;
input bool   InpUseEMAPullback    = true;            // V12: pullback as setup gate
input int    InpPullbackBars      = 4;
input bool   InpRequirePullbackAND= true;            // V12: NEW — require BOTH pattern AND pullback

input group "═══ Pattern Quality ═══════════════════════════════"
input double InpMinBodyATR        = 0.40;            // Min pattern body size as fraction of ATR (V2 tightened)
input double InpPinWickRatio      = 3.0;             // V8: very strict
input double InpPinOppWickMaxR    = 0.20;            // V8: very strict
...
input bool   InpUseRSI            = false;           // V9: disabled (pullback is the setup, RSI redundant)
...
input double InpMinADX            = 22.0;            // V9: relaxed (pullbacks need active but not extreme trend)
...
input double InpMaxDailyLossPct   = 4.0;             // V14: tighter daily loss cap             // Max daily loss % of start-of-day balance
input bool   InpUseLossBreaker    = true;
input int    InpMaxConsecLosses   = 3;               // V6: tighter (was 4)               // Halt for the day after N consec losses
input bool   InpUseCooldown       = true;
input int    InpCooldownBars      = 6;               // Cooldown bars (M5) after a loss

input group "═══ Daily Trade Limit (V8) ═══════════════════════"
input int    InpMaxTradesPerDay   = 4;               // V9: relaxed

input group "═══ Time Stop ═════════════════════════════════════"
input bool   InpUseTimeStop       = false;           // V4: DISABLED — let SL/TP work
input int    InpMaxHoldBars       = 96;              // Fallback if enabled (96×5m = 8h)
...
// V4: Trade management state (per active position)
...
// V3: Pattern pending breakout confirmation
struct PendingPattern
{
   bool     active;
   int      dir;
   string   name;
   double   extreme;       // SL anchor (pattern low for bull, high for bear)
   double   trigger;       // breakout level (pattern high for bull, low for bear)
   datetime detected_at;
   int      bars_remaining;// expires after N bars without breakout
};
PendingPattern g_pending = {false, 0, "", 0.0, 0.0, 0, 0};
...
bool IsDailyLossBreached()
{
   ...
   if(today != g_today)
   {
      ...
      g_today_trades    = 0;          // V8: reset daily trade counter
      return false;
   }
...
struct PatternHit
{
   ...
   double trigger;   // V3: breakout confirmation level (pattern high for bull, low for bear)
};
...
bool DetectStar(double atr, PatternHit &hit)
{
      ...
      b1.c > (b3.o + b3.c) * 0.5)
   {
      ...
      hit.trigger = MathMax(MathMax(b1.h, b2.h), b3.h);
      return true;
   }
   // Evening star
      ...
      b1.c < (b3.o + b3.c) * 0.5)
   {
      ...
      hit.trigger = MathMin(MathMin(b1.l, b2.l), b3.l);
      return true;
   }
   return false;
}
...
// V9: EMA pullback continuation
// Long: HTF uptrend + bar1 closed back above M5 EMA20 with bullish body,
//       AND in last InpPullbackBars some bar's low touched/crossed below EMA20.
bool DetectEMAPullback(double atr, int htf_dir, PatternHit &hit)
{
   if(htf_dir == 0) return false;

   Bar b1;
   if(!LoadBar(1, b1)) return false;
   double ema_at_1 = 0.0;
   if(!ReadBuffer(g_h_fast, 1, ema_at_1)) return false;
   double atr_floor = atr * InpMinBodyATR;

   // Look for pullback in last N bars
   bool pulled_back = false;
   double swing_low = b1.l, swing_high = b1.h;
   for(int s = 1; s <= InpPullbackBars; s++)
   {
      ...
      if(htf_dir > 0 && bx.l <= ema_s) pulled_back = true;
      if(htf_dir < 0 && bx.h >= ema_s) pulled_back = true;
      if(bx.l < swing_low)  swing_low  = bx.l;
      if(bx.h > swing_high) swing_high = bx.h;
   }
   if(!pulled_back) return false;

   // Long: bar1 closes above EMA with bullish body
   if(htf_dir > 0 && Bull(b1) && b1.c > ema_at_1 && Body(b1) >= atr_floor)
   {
      hit.dir     = +1;
      hit.name    = "EMAPullbackLong";
      hit.extreme = swing_low;       // SL anchored to swing low of pullback
      hit.trigger = b1.h;
      return true;
   }
   // Short
   if(htf_dir < 0 && Bear(b1) && b1.c < ema_at_1 && Body(b1) >= atr_floor)
   {
      hit.dir     = -1;
      hit.name    = "EMAPullbackShort";
      hit.extreme = swing_high;
      hit.trigger = b1.l;
      return true;
   }
   return false;
...
bool OpenTrade(int dir, double extreme, double atr, string pname)
{
   ...
   double max_dist = atr * InpMaxSLDistATR;
   double sl, tp;
   if(dir > 0)
   {
      sl = NormalizeDouble(extreme - buffer, digits);
      // V3: cap SL distance at MaxSLDistATR
      if(entry - sl > max_dist) sl = NormalizeDouble(entry - max_dist, digits);
   ...
   else
   {
      sl = NormalizeDouble(extreme + buffer, digits);
      // V3: cap SL distance at MaxSLDistATR
      if(sl - entry > max_dist) sl = NormalizeDouble(entry + max_dist, digits);
   ...
   {
      g_today_trades++;
      LogInfo(StringFormat("%s %s %.2f @ %.2f SL=%.2f TP=%.2f (risk=%.1f%% lot=%.2f) #%d/day",
              dir > 0 ? "BUY" : "SELL", pname, vol, entry, sl, tp, InpRiskPercent, vol, g_today_trades));
      // Capture the just-opened ticket so we can record P/L when it closes
      ulong t = 0; long ty = -1; datetime ot = 0;
      if(FindOurPosition(t, ty, ot))
      {
         g_last_open_ticket = t;
         // V4: arm trade-management state
         g_managed_ticket   = t;
         g_initial_sl_dist  = MathAbs(entry - sl);
         g_entry_price      = (dir > 0) ? PositionGetDouble(POSITION_PRICE_OPEN) : PositionGetDouble(POSITION_PRICE_OPEN);
         g_pos_dir          = dir;
...
//+------------------------------------------------------------------+
//| V4: Trade management — partial close + break-even                 |
//+------------------------------------------------------------------+
void ManagePosition(ulong ticket)
{
   if(g_managed_ticket != ticket || g_initial_sl_dist <= 0.0) return;
   ...
   double favor_dist = 0.0;
   if(g_pos_dir > 0) favor_dist = cur_bid - g_entry_price;
   else              favor_dist = g_entry_price - cur_ask;

   if(favor_dist <= 0.0) return;
   double r_progress = favor_dist / g_initial_sl_dist;

   // ── Partial close at the trigger ──
   if(InpUsePartialClose && !g_partial_done && r_progress >= InpPartialAtR)
   {
      double partial_vol = NormalizeVol(vol * InpPartialFraction);
      double remaining = vol - partial_vol;
      // Skip partial if it would leave us below min volume
      double minv = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
      if(partial_vol >= minv && remaining >= minv)
      {
         if(g_trade.PositionClosePartial(ticket, partial_vol))
         {
            g_partial_done = true;
            LogInfo(StringFormat("Partial close %.2f at %.1fR (entry=%.2f price=%.2f)",
                    partial_vol, r_progress,
                    g_entry_price,
                    g_pos_dir > 0 ? cur_bid : cur_ask));
         }
      }
      else
      {
         // Position too small to split — just mark partial as done so BE still triggers
         g_partial_done = true;
         LogDebug("Skipped partial — volume too small to split");
      }
   }

   // ── Break-even SL once partial is done (or volume too small) ──
   if(InpUseBreakEven && !g_be_done && g_partial_done)
   {
      double offset = g_initial_sl_dist * InpBreakEvenOffsetR;
      double new_sl;
      if(g_pos_dir > 0)
      {
         new_sl = NormalizeDouble(g_entry_price + offset, digits);
         if(new_sl > cur_sl)
         {
            if(g_trade.PositionModify(ticket, new_sl, cur_tp))
            {
               g_be_done = true;
               LogInfo(StringFormat("BE: SL moved to %.2f (entry+%.1f%%R)", new_sl, InpBreakEvenOffsetR*100));
            }
         }
      }
      else
...
void OnTick()
{
   ...
   if(has_pos)
   {
      ...
      ManagePosition(ticket);          // V4: partial + BE
   ...
   if(g_last_open_ticket != 0)
   {
      ...
      // V4: reset trade-mgmt state
      g_managed_ticket  = 0;
      g_initial_sl_dist = 0.0;
      g_partial_done    = false;
      g_be_done         = false;
   }

   // ── V3: First, check for active pending pattern breakout (every tick) ──
   if(g_pending.active)
   {
      double cur_bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      double cur_ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double atr_now = 0.0;
      if(!ReadBuffer(g_h_atr, 0, atr_now) || atr_now <= 0.0) return;

      bool triggered = false;
      if(g_pending.dir > 0 && cur_ask > g_pending.trigger) triggered = true;
      if(g_pending.dir < 0 && cur_bid < g_pending.trigger) triggered = true;

      if(triggered)
      {
         // Final risk gates at trigger time
         if(IsSessionAllowed() && !IsDailyLossBreached() &&
            !IsCooldownActive() && !IsLossBreakerActive() && IsSpreadOK())
         {
            OpenTrade(g_pending.dir, g_pending.extreme, atr_now, g_pending.name + "-BO");
         }
         g_pending.active = false;
         return;
      }
      // Decrement countdown on new bar
      if(new_bar)
      {
         g_pending.bars_remaining--;
         if(g_pending.bars_remaining <= 0)
         {
            LogDebug(StringFormat("Pending %s expired", g_pending.name));
            g_pending.active = false;
         }
      }
   }

   // Only scan for new patterns on new-bar close
   if(!new_bar) return;
   ...
   if(IsDailyTradeLimitReached()){ LogDebug("Skip: daily-trade-cap"); return; }
   ...
   // ── V12: confluence scan ──
   PatternHit hit_p, hit_pb;
   hit_p.dir  = 0; hit_p.extreme  = 0.0; hit_p.name  = ""; hit_p.trigger  = 0.0;
   hit_pb.dir = 0; hit_pb.extreme = 0.0; hit_pb.name = ""; hit_pb.trigger = 0.0;

   bool has_pat = ScanPatterns(atr, hit_p);
   bool has_pb  = InpUseEMAPullback ? DetectEMAPullback(atr, htf_dir, hit_pb) : false;

   PatternHit hit;
   if(InpRequirePullbackAND)
   {
      // Need BOTH and same direction
      if(!has_pat || !has_pb) return;
      if(hit_p.dir != hit_pb.dir) return;
      hit       = hit_p;
      hit.name  = StringFormat("%s+Pullback", hit_p.name);
      // Use the wider of the two SL anchors (safer)
      if(hit_p.dir > 0) hit.extreme = MathMin(hit_p.extreme, hit_pb.extreme);
      else              hit.extreme = MathMax(hit_p.extreme, hit_pb.extreme);
   }
   else
   {
      if(has_pat)        hit = hit_p;
      else if(has_pb)    hit = hit_pb;
      else               return;
   }
   ...
   // ── V3: Either enter immediately or arm pending breakout ──
   if(InpRequireBreakout)
   {
      g_pending.active         = true;
      g_pending.dir            = hit.dir;
      g_pending.name           = hit.name;
      g_pending.extreme        = hit.extreme;
      g_pending.trigger        = hit.trigger;
      g_pending.detected_at    = TimeCurrent();
      g_pending.bars_remaining = 3;     // expires after 3 bars
      LogDebug(StringFormat("Armed %s pending breakout at %.2f", hit.name, hit.trigger));
   }
   else
   {
