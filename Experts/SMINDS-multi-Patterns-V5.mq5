...
input long   InpMagicNumber       = 95001005;        // Magic number
input string InpOrderComment      = "SMINDS-MP-V5";  // Order comment
...
input double InpRiskPercent       = 1.0;             // Risk % of equity per trade (V5: lowered)
...
input double InpRRRatio           = 2.0;             // TP distance = SL distance × this (V4: 2:1 baseline)

input group "═══ Trade Management (V4) ════════════════════════"
input bool   InpUsePartialClose   = false;           // V5: disabled — clean 2:1 SL/TP
input double InpPartialAtR        = 1.0;             // (unused when partial off)
input double InpPartialFraction   = 0.5;             // (unused when partial off)
input bool   InpUseBreakEven      = false;           // V5: disabled — avoid premature stops
input double InpBreakEvenOffsetR  = 0.1;             // (unused)
...
input int    InpMaxConsecLosses   = 3;               // Halt for the day after N consec losses (V5: tighter)
input bool   InpUseCooldown       = true;
input int    InpCooldownBars      = 12;              // Cooldown bars after a loss (V5: longer)
