...
# Delete cached .set so MT5 uses compiled defaults (otherwise saved inputs override new compile)
$expertBaseName = [System.IO.Path]::GetFileNameWithoutExtension($ExpertFile)
$setFile = "$dataDir\MQL5\Profiles\Tester\$expertBaseName.set"
if (Test-Path $setFile) {
    Remove-Item $setFile -Force
    Write-Host "Deleted cached .set: $setFile" -ForegroundColor Yellow
}
